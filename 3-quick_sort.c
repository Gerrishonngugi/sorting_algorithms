...
int partition(int *array, int beg, int end, int size)
{
	...
	/* set partition index as start point */
...
/**
 * quick_sort - sorts array of integers in ascending order using the,
...
{
	/* Array doesnt need to be sorted if its size is less than 2 */
	if (!array || size < 2)
		return;
